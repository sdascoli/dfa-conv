...
class DFA(nn.Module):
    def __init__(self, dfa_layers, normalization=True, rp_device=None, no_training=False,
                 feedback_points_handling=FeedbackPointsHandling.LAST, batch_dims=(0,), B=None):
    ...
    def forward(self, input):
        if not(self.initialized or self.no_training):
            ...
            if self.feedback_matrix is None:
                ...
                self.feedback_matrix = torch.rand(len(self.dfa_layers),self.output_size, self.max_feedback_size, device=self.rp_device) * 2 - 1
